...
@section('content')
    @if ($user->role_id == 1)
        <h3 class="page-title">{{$title}}
            &nbsp; &nbsp;<a href="{{url('/admin/allkids')}}" class="btn btn-primary">Rodyti visus vaikus</a>
        </h3>
    @endif
    @can('kid_create')
        <p>
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped {{ count($kids) > 0 ? 'datatable' : '' }} @can('kid_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                <tr>
                    @can('kid_delete')
                ...
                @if (count($kids) > 0)
                    @foreach ($kids as $kid)
                        <tr data-entry-id="{{ $kid->id }}">
                            ...
                            <td field-key='ifsc_licence'>{{ Form::checkbox("ifsc_licence", 1, $kid->ifsc_licence == 1 ? true : false, ["disabled"]) }}</td>
